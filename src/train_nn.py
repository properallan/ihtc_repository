...
def orderReduction(low_fidelity_data, high_fidelity_data, rank):
    bounds = [0,1]

    q1d = SVD(low_fidelity_data)
    q1d.normalize(bounds=bounds)
    q1d.subtractMean()
    q1d.SVD()
    q1d.setRank(rank)
    #rank = q1d.findRank(99.00)
...
def trainNN_thickness(q1d, thicknesss, su2, rank):
...
def trainNN(q1d, su2, rank):

    model = get_model(rank, rank, 10, rank*10)
    ...
    q1d_train = q1d.L[:ntrain,...]
    su2_train = su2.L[:ntrain,...]
   
    q1d_test = q1d.L[ntrain:ntest,...]
    su2_test = su2.L[ntrain:ntest,...]
    
    q1d_validation = q1d.L[ntest:nvalid,...]
    su2_validation = su2.L[ntest:nvalid,...]
...
def main():
    ...
    model = trainNN(q1d, su2, rank)
    model.save('/home/ppiper/Dropbox/local/ihtc_nozzle/data/doe_lhs_N200/trained_nn')
