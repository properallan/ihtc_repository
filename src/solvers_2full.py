...
class nozzle:
    ...
    def genMultizone(self, outputpath, fnames):
        ...
        # mesh dimension
        ndim = 2
        create_zone_meshes = 0

        #-------------------------------------------------------------------#
        # Create .su2 meshes out of .geo files with gmsh
        if create_zone_meshes:
            for file in geo_files:
                #create_mesh = [f'{gmshsolver}', '-3', '-smooth', '5' ,'-optimize', outputpath +file+'.geo', '-2', '-f', 'su2', '-o', outputpath+file+'.su2']
                create_mesh = [f'{gmshsolver}' , outputpath +file+'.geo', '-2', '-f', 'su2', '-o', outputpath+file+'.su2']
    ...
    def genGmshWall(self, outputpath, fname, thickness):
        ...
        p = subprocess.Popen([f'{gmshsolver}', '-3', '-smooth', '5' ,'-optimize', geofilename,'-0','-2','-format','su2','-o', outputpath +fname+'.su2'])
        p.wait()
        p = subprocess.Popen([f'{gmshsolver}', '-3', '-smooth', '5' ,'-optimize', geofilename,'-0','-2','-format','msh','-o', outputpath +fname+'.msh'])
        p.wait()
        p = subprocess.Popen([f'{gmshsolver}', '-3', '-smooth', '5' ,'-optimize', geofilename,'-0','-2','-format','msh','-o', outputpath +fname+'.vtk'])
        p.wait()
    ...
    def genGmshWall_1(self, outputpath, fname, thickness):
        x = self.xn
        r = (self.Sn/np.pi)**0.5
        Nx = self.NxWall
        Ny = self.NyWall
        ...
        geo.addPhysicalLine("INNERWALL",l1_2)
        geo.addPhysicalLine("OUTERWALL",l2_2)
        geo.addPhysicalLine("INWALL",l3_2)
        geo.addPhysicalLine("OUTWALL",l4_2)
    ...
    def genGmshNozzle(self, outputpath, fname):
        ...
        p = subprocess.Popen([f'{gmshsolver}', geofilename,'-0','-2','-format','su2','-o', outputpath +fname+'.su2'])
        p.wait()
        p = subprocess.Popen([f'{gmshsolver}', geofilename,'-0','-2','-format','msh','-o', outputpath +fname+'.msh'])
        p.wait()
        p = subprocess.Popen([f'{gmshsolver}', geofilename,'-0','-2','-format','msh','-o', outputpath +fname+'.vtk'])
        p.wait()
    ...
    def solveSU2(self, solver, cores=None, cwd=None):
...
% -------------------- BOUNDARY CONDITION DEFINITION --------------------------%
%
MARKER_ISOTHERMAL= ( OUTERWALL, {self.outerTemperature}, OUTERWALL_2, {self.outerTemperature} ) 

MARKER_HEATFLUX= ( INWALL, 0.0, OUTWALL, 0.0, INWALL_2, 0.0, OUTWALL_2, 0.0)

% Marker(s) of the surface to be plotted or designed
MARKER_PLOTTING= ( INNERWALL, INNERWALL_2)
%
% Marker(s) of the surface where the functional (Cd, Cl, etc.) will be evaluated
MARKER_MONITORING= ( INNERWALL, INNERWALL_2 )
...
"""
        ...
        #self.genGmshWall_1(self.su2infilepath, 'solid_setupSU2.cfg', self.thickness) 
        ...
        self.genMultizone(self.su2infilepath, ['setupSU2','solid_setupSU2'])
