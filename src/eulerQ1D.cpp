    ...
    public:
        ...
        void solve(){
            ...
            for (int it = 1; it < itmax+1 && maxres > tol ; it++)
            {
                ...
                resit.row(it) = dU.block(1,0,U.rows()-2,U.cols()).array().abs().colwise().maxCoeff();
                    

                if (it%itprint == 0){
                    cout << it << "\t\t" << resit(it,0) << "\t" << resit(it,1) << "\t" << resit(it,2) << endl;
                    outfile << it << "\t\t" << resit(it,0) << "\t" << resit(it,1) << "\t" << resit(it,2) << endl;
                }

                maxres = resit.row(it).maxCoeff();
            }
